    ...
    def build_flow_loss_final(self):
        ...
        for s in range(self.num_scales):
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src0 * occu_masks_bw[3][s]) / occu_masks_bw_avg[3][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src0_1 * occu_masks_bw[5][s]) / occu_masks_bw_avg[5][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src0_2 * occu_masks_bw[7][s]) / occu_masks_bw_avg[7][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src0_3 * occu_masks_bw[9][s]) / occu_masks_bw_avg[9][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src1 * occu_masks_bw[0][s]) / occu_masks_bw_avg[0][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src1_1 * occu_masks_bw[2][s]) / occu_masks_bw_avg[2][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src1_2 * occu_masks_fw[3][s]) / occu_masks_fw_avg[3][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src1_3 * occu_masks_bw[8][s]) / occu_masks_bw_avg[8][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_tgt * occu_masks_fw[0][s]) / occu_masks_fw_avg[0][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_tgt_1 * occu_masks_bw[1][s]) / occu_masks_bw_avg[1][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_tgt_2 * occu_masks_fw[5][s]) / occu_masks_fw_avg[5][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_tgt_3 * occu_masks_bw[6][s]) / occu_masks_bw_avg[6][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src2 * occu_masks_fw[1][s]) / occu_masks_fw_avg[1][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src2_1 * occu_masks_bw[4][s]) / occu_masks_bw_avg[4][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src2_2 * occu_masks_fw[2][s]) / occu_masks_fw_avg[2][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src2_3 * occu_masks_fw[7][s]) / occu_masks_fw_avg[7][s]
            ...
            reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src3 * occu_masks_fw[4][s]) / occu_masks_fw_avg[4][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src3_1 * occu_masks_fw[6][s]) / occu_masks_fw_avg[6][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src3_2 * occu_masks_fw[8][s]) / occu_masks_fw_avg[8][s]
            ...
            cross_reconstructed_loss += tf.reduce_mean(
                input_tensor=curr_proj_error_optical_src3_3 * occu_masks_fw[9][s]) / occu_masks_fw_avg[9][s]

            if self.ssim_weight > 0:
                # src0
                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src0 * occu_masks_bw[3][s],
                         curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[3][s])) / occu_masks_bw_avg[3][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src0_1 * occu_masks_bw[5][s],
                         curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[5][s])) / occu_masks_bw_avg[5][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src0_2 * occu_masks_bw[7][s],
                         curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[7][s])) / occu_masks_bw_avg[7][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src0_3 * occu_masks_bw[9][s],
                         curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[9][s])) / occu_masks_bw_avg[9][s]

                # src1
                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src1 * occu_masks_bw[0][s],
                         curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[0][s])) / occu_masks_bw_avg[0][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src1_1 * occu_masks_bw[2][s],
                         curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[2][s])) / occu_masks_bw_avg[2][s]

                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src1_2 * occu_masks_fw[3][s],
                         curr_src_image_stack[:,:,:,3:6] * occu_masks_fw[3][s])) / occu_masks_fw_avg[3][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src1_3 * occu_masks_bw[8][s],
                         curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[8][s])) / occu_masks_bw_avg[8][s]

                # tgt
                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_tgt * occu_masks_fw[0][s],
                         curr_tgt_image * occu_masks_fw[0][s])) / occu_masks_fw_avg[0][s]

                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_tgt_1 * occu_masks_bw[1][s],
                         curr_tgt_image * occu_masks_bw[1][s])) / occu_masks_bw_avg[1][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_tgt_2 * occu_masks_fw[5][s],
                         curr_tgt_image * occu_masks_fw[5][s])) / occu_masks_fw_avg[5][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_tgt_3 * occu_masks_bw[6][s],
                         curr_tgt_image * occu_masks_bw[6][s])) / occu_masks_bw_avg[6][s]

                # src2
                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src2 * occu_masks_fw[1][s],
                         curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[1][s])) / occu_masks_fw_avg[1][s]

                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src2_1 * occu_masks_bw[4][s],
                         curr_src_image_stack[:,:,:,6:9] * occu_masks_bw[4][s])) / occu_masks_bw_avg[4][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src2_2 * occu_masks_fw[2][s],
                         curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[2][s])) / occu_masks_fw_avg[2][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src2_3 * occu_masks_fw[7][s],
                         curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[7][s])) / occu_masks_fw_avg[7][s]

                # src3
                ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src3 * occu_masks_fw[4][s],
                         curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[4][s])) / occu_masks_fw_avg[4][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src3_1 * occu_masks_fw[6][s],
                         curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[6][s])) / occu_masks_fw_avg[6][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src3_2 * occu_masks_fw[8][s],
                         curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[8][s])) / occu_masks_fw_avg[8][s]

                cross_ssim_loss += tf.reduce_mean(
                    input_tensor=SSIM(curr_proj_image_optical_src3_3 * occu_masks_fw[9][s],
        ...
        tf.compat.v1.summary.image('scale%d_flow_src12tgt' % s, fl.flow_to_color(self.pred_fw_flows[0][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_tgt2src2' % s, fl.flow_to_color(self.pred_fw_flows[1][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_src12src2' % s, fl.flow_to_color(self.pred_fw_flows[2][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_src02src1' % s, fl.flow_to_color(self.pred_fw_flows[3][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_src22src3' % s, fl.flow_to_color(self.pred_fw_flows[4][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_src02tgt' % s, fl.flow_to_color(self.pred_fw_flows[5][s], max_flow=256))
        tf.compat.v1.summary.image('scale%d_flow_tgt2src3' % s, fl.flow_to_color(self.pred_fw_flows[6][s], max_flow=256))
    ...
    def build_dp_loss_mask_allpose(self):
        ...
        for s in range(self.num_scales):
            ...
            if not self.compute_minimum_loss:
                # src0
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src0 * occu_masks_bw[3][selected_scale]) / occu_masks_bw_avg[3][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src0_1 * occu_masks_bw[5][selected_scale]) / occu_masks_bw_avg[5][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src0_2 * occu_masks_bw[7][selected_scale]) / occu_masks_bw_avg[7][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src0_3 * occu_masks_bw[9][selected_scale]) / occu_masks_bw_avg[9][selected_scale]
                # src1
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src1 * occu_masks_bw[0][selected_scale]) / occu_masks_bw_avg[0][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src1_1 * occu_masks_bw[2][selected_scale]) / occu_masks_bw_avg[2][selected_scale]
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src1_2 * occu_masks_fw[3][selected_scale]) / occu_masks_fw_avg[3][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src1_3 * occu_masks_bw[8][selected_scale]) / occu_masks_bw_avg[8][selected_scale]
                # tgt
                ...
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_tgt_1 * occu_masks_bw[6][selected_scale]) / occu_masks_bw_avg[6][selected_scale]
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_tgt_2 * occu_masks_fw[0][selected_scale]) / occu_masks_fw_avg[0][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_tgt_3 * occu_masks_fw[5][selected_scale]) / occu_masks_fw_avg[5][selected_scale]
                # src2
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src2 * occu_masks_bw[4][selected_scale]) / occu_masks_bw_avg[4][selected_scale]
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src2_1 * occu_masks_fw[1][selected_scale]) / occu_masks_fw_avg[1][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src2_2 * occu_masks_fw[2][selected_scale]) / occu_masks_fw_avg[2][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src2_3 * occu_masks_fw[7][selected_scale]) / occu_masks_fw_avg[7][selected_scale]
                # src3
                reconstructed_loss +=       tf.reduce_mean(input_tensor=curr_proj_error_src3 * occu_masks_fw[4][selected_scale]) / occu_masks_fw_avg[4][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src3_1 * occu_masks_fw[6][selected_scale]) / occu_masks_fw_avg[6][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src3_2 * occu_masks_fw[8][selected_scale]) / occu_masks_fw_avg[8][selected_scale]
                cross_reconstructed_loss += tf.reduce_mean(input_tensor=curr_proj_error_src3_3 * occu_masks_fw[9][selected_scale]) / occu_masks_fw_avg[9][selected_scale]

                if self.ssim_weight > 0:
                    # src0
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src12src0 * occu_masks_bw[3][selected_scale], curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[3][selected_scale])) / occu_masks_bw_avg[3][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_tgt2src0 * occu_masks_bw[5][selected_scale], curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[5][selected_scale])) / occu_masks_bw_avg[5][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src22src0 * occu_masks_bw[7][selected_scale], curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[7][selected_scale])) / occu_masks_bw_avg[7][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src32src0 * occu_masks_bw[9][selected_scale], curr_src_image_stack[:,:,:,0:3] * occu_masks_bw[9][selected_scale])) / occu_masks_bw_avg[9][selected_scale]
                    # src1
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_tgt2src1 * occu_masks_bw[0][selected_scale], curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[0][selected_scale])) / occu_masks_bw_avg[0][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src22src1 * occu_masks_bw[2][selected_scale], curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[2][selected_scale])) / occu_masks_bw_avg[2][selected_scale]
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src02src1 * occu_masks_fw[3][selected_scale], curr_src_image_stack[:,:,:,3:6] * occu_masks_fw[3][selected_scale])) / occu_masks_fw_avg[3][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src32src1 * occu_masks_bw[8][selected_scale], curr_src_image_stack[:,:,:,3:6] * occu_masks_bw[8][selected_scale])) / occu_masks_bw_avg[8][selected_scale]
                    # tgt
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src22tgt * occu_masks_bw[1][selected_scale], curr_tgt_image * occu_masks_bw[1][selected_scale])) / occu_masks_bw_avg[1][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src32tgt * occu_masks_bw[6][selected_scale], curr_tgt_image * occu_masks_bw[6][selected_scale])) / occu_masks_bw_avg[6][selected_scale]
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src12tgt * occu_masks_fw[0][selected_scale], curr_tgt_image * occu_masks_fw[0][selected_scale])) / occu_masks_fw_avg[0][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src02tgt * occu_masks_fw[5][selected_scale], curr_tgt_image * occu_masks_fw[5][selected_scale])) / occu_masks_fw_avg[5][selected_scale]
                    # src2
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src32src2 * occu_masks_bw[4][selected_scale], curr_src_image_stack[:,:,:,6:9] * occu_masks_bw[4][selected_scale])) / occu_masks_bw_avg[4][selected_scale]
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_tgt2src2 * occu_masks_fw[1][selected_scale], curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[1][selected_scale])) / occu_masks_fw_avg[1][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src12src2 * occu_masks_fw[2][selected_scale], curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[2][selected_scale])) / occu_masks_fw_avg[2][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src02src2 * occu_masks_fw[7][selected_scale], curr_src_image_stack[:,:,:,6:9] * occu_masks_fw[7][selected_scale])) / occu_masks_fw_avg[7][selected_scale]
                    # src3
                    ssim_loss +=       tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src22src3 * occu_masks_fw[4][selected_scale], curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[4][selected_scale])) / occu_masks_fw_avg[4][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_tgt2src3 * occu_masks_fw[6][selected_scale], curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[6][selected_scale])) / occu_masks_fw_avg[6][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src12src3 * occu_masks_fw[8][selected_scale], curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[8][selected_scale])) / occu_masks_fw_avg[8][selected_scale]
                    cross_ssim_loss += tf.reduce_mean(input_tensor=SSIM(curr_proj_image_src02src3 * occu_masks_fw[9][selected_scale], curr_src_image_stack[:,:,:,9:12] * occu_masks_fw[9][selected_scale])) / occu_masks_fw_avg[9][selected_scale]
            ...
            if self.dp_smooth_weight > 0:
                if self.depth_normalization:
                    ...
                    mean_src2_disp = tf.reduce_mean(input_tensor=self.disp['src2'][s], axis=[1, 2, 3], keepdims=True)
                    src2_disp_input = self.disp['src2'][s] / mean_src2_disp
                    mean_src3_disp = tf.reduce_mean(input_tensor=self.disp['src3'][s], axis=[1, 2, 3], keepdims=True)
        ...
        tf.compat.v1.summary.image('scale%d_src2_pred_disp' % s, self.disp['src2'][s])
        tf.compat.v1.summary.image('scale%d_src3_pred_disp' % s, self.disp['src3'][s])
        ...
        tf.compat.v1.summary.image('occlusion_tgt', flyout_map_all_tgt[0])
        tf.compat.v1.summary.image('occlusion_src0', flyout_map_all_src0[0])
        tf.compat.v1.summary.image('occlusion_src1', flyout_map_all_src1[0])
        tf.compat.v1.summary.image('occlusion_src2', flyout_map_all_src2[0])
        tf.compat.v1.summary.image('occlusion_src3', flyout_map_all_src3[0])
