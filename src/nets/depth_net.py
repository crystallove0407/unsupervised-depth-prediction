...
def D_Net(dispnet_inputs, weight_reg=0.0004, is_training=False, reuse=False):
    D_Model = ShuffleNetV2(input_holder=dispnet_inputs, var_scope='depth_net', model_scale=1.0, shuffle_group=2, is_training=is_training, decoderType='separable')
...
# ShuffleNetV2
###################################################################################
# call function: conv_bn_relu, shufflenet_v2_block, upsample, dec_block, get_pred
# decoderType : separable, mobile, shuffle
###################################################################################
            ...
            with slim.arg_scope([slim.batch_norm], is_training=self.is_training):
                ...
                with tf.variable_scope('decoder'):
                    ...
                    for idx, channel in enumerate(self.output_channel_sizes):
                        x = upsample(x, 2)
                        x = dec_block(x, channel, scope='up_block_%d' % idx, decoderType='separable')
                        ...
                        x = dec_block(x, channel, scope='block_%d' % idx, decoderType='separable')
...
def dec_block(input_tensor, output_channel, scope = None, decoderType='separable'):
    # call function: slim.separable_conv2d, slim.conv2d, sufflenet_v2_block
    with tf.variable_scope(scope, default_name='dec_block'):
        ...
        if decoderType == 'separable':
            ...
            net = shufflenet_v2_block(net, output_channel, kernel_size=3, stride=1, dilation=1, shuffle_group=2)
...
def shufflenet_v2_block(x, out_channel, kernel_size, stride=1, dilation=1, shuffle_group=2):
    ...
    with tf.variable_scope(None, 'shuffle_v2_block'):
        ...
        if stride == 1 and x.shape[-1] == out_channel:
            ...
            top = conv_bn_relu(top, half_channel, kernel_size=1)
        ...
        # stride == 2
            ...
            b0 = conv_bn_relu(x, half_channel, kernel_size=1)
            b0 = depthwise_conv_bn(b0, kernel_size, stride, dilation)
            b0 = conv_bn_relu(b0, half_channel, kernel_size=1)

            b1 = depthwise_conv_bn(x, kernel_size, stride, dilation)
            b1 = conv_bn_relu(b1, half_channel, kernel_size=1)
...
def conv_bn_relu(x, out_channel, kernel_size, stride=1, dilation=1):
    with tf.variable_scope(None, 'conv_bn_relu'):
        ...
        x = slim.batch_norm(x, activation_fn=tf.nn.relu6, fused=False)
    return x
...
def get_pred(x):    
    ...
    x = slim.conv2d(x, 1, 3, 1, 'VALID', activation_fn=tf.nn.elu, normalizer_fn=slim.batch_norm)
    disp = 10 * x + 0.01
    return disp
